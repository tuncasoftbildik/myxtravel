...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    fill: true,
    anim: "animate-icon-drive",
    bg: "/menu/transfer.jpg",
  },
  {
    label: "Kampanyalar",
    href: "/kampanyalar",
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-8 sm:pt-10 pb-8 sm:pb-10">
        ...
        <nav className="flex items-stretch justify-center gap-4 sm:gap-6 md:gap-8 flex-nowrap">
          {NAV_ITEMS.map((item) => (
            ...
            >
              <div
                className="relative w-[88px] h-[88px] sm:w-[108px] sm:h-[108px] rounded-2xl shadow-md overflow-hidden group-hover:shadow-xl group-hover:shadow-black/20 transition-all duration-300"
              >
                <div
                  className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
                  style={{ backgroundImage: `url(${item.bg})` }}
                />
                <div className="absolute inset-0 bg-black/40 group-hover:bg-brand-red/60 transition-all duration-300" />
                <div className="relative w-full h-full flex items-center justify-center">
                  <svg
                    className={`w-11 h-11 sm:w-12 sm:h-12 text-white drop-shadow-lg transition-all duration-300 group-hover:scale-110 ${item.anim}`}
